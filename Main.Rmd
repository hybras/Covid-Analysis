...
NOTE: All code blocks are folded by default. Access by clicking code button next to the text block. 
(Or open all at once with the button on the top right)
...
plot_2 <- data_scaled %>%
	...
	labs(title = "Confirmed Cases per 100,000 People vs Time",
		 x = "Date",
		 y = "Number of Cases") +
	geom_line() +
	# Assuming the data goes up to yesterday.
...
```{r}
plot_legislation_overlay <- function (state, legislation, statecolor, legislationcolor) {
  # first letter of state should be capatialized (eg. "Maryland")
  # legislation is neumeric - see following
  ...
  legislation_matrix = c("stay at home order", "mandatory quarantine for travelers", "non essential business closures", "large gatherings ban", "school closures", "restaurant limits")
  ...
  if(les_start == "-") { # legislation was never enacted
    not_enacted = TRUE
  } else if(les_end == "-") { # legislation was never lifted
  ...
  plot <- state_data %>% 
    ...
    labs(title = paste(paste(paste("COVID case data with", legislation_matrix[legislation], sep=" "), "overlayed -", sep=" "), state, sep=" "),
         x = "Date",
         y = "Number of Cases") +
    geom_line(color = statecolor) + 
    scale_x_date(limits = c(as.Date("2020-3-01"), as.Date("2020-7-31")),
                 date_breaks = "1 month",
                 date_labels = "%B") +
    scale_y_continuous(labels = scales::comma) +
    theme(
  ...
  if(not_enacted) plot else plot + annotate("rect", xmin=les_start, xmax=les_end, ymin=0, ymax=Inf, alpha=0.2, fill=legislationcolor)
}
```
