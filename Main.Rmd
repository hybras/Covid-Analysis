...
#####################################################
# Helper functions to generate plots. Can be animated.
plot_cases <-
	function(cases,
			 scaled = FALSE,
...
# This function needs the plot argument to be passed in in the form (plot = p + transition_reveal(argument_to_animate))
animate_plot <-
...
animate_plot(plot_1 + transition_reveal(as.Date(date)))
```
...
animate_plot(plot_2 + transition_reveal(as.Date(date)))
```
...
path <- "./data/lockdownTrendData.csv"
lockdown_data <- read.csv(path)
date <- as.Date(lockdown_data$Week, "%Y-%m-%d") # variable name has to be "date" to match animation

plot_3 <- ggplot() +
  labs(title = "Confirmed Cases per 100,000 People vs Time and Lockdown Trend",
		 x = "Date",
		 y = "Number of Cases") +
  geom_line(data = lockdown_data, aes(x = lockdown_dates, y =lockdown * 25), size = 2) +
  geom_line(data = data_scaled, aes(
		x = as.Date(date),
		y = scaled_cases,
		group = region,
		color = region
...
animate_plot(plot_3 + transition_reveal(as.Date(date)))
```
