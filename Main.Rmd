...
plot_cases <- function(cases, scaled = FALSE, start_date=as.Date("2020-1-22"), end_date=Sys.Date() - 1) {
	plot_1 <- cases %>% ggplot(aes(
		x = as.Date(date),
		y = if (scaled) scaled_cases else cases,
		group = region,
		color = region
	)) +
...
plot_legislation_overlay <-
	function (state,
		...
		plot <- state_data %>%
			...
			scale_x_date(
				limits = c(as.Date("2020-3-01"), as.Date("2020-7-31")),
				date_breaks = "1 month",
				date_labels = "%B"
			) +
			scale_y_continuous(labels = scales::comma) +
