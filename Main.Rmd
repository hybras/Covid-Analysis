...
# For each state, find that states population, divide all of the case counts by the population and merge the scaled data back into the dataframe
for (state in cont_states) {
  data_scaled <- by_state.long_1 %>%
    filter(region == state) %>%
    mutate(scaled_cases = (cases / filter(census, NAME == state)$POPESTIMATE2019) * 100000) %>%
    merge(data_scaled, all = TRUE)
}

fav_states_scaled <- data_scaled %>% filter(region == fav_states)
```

Scaled plots.

```{r scaled_gif}
plot_2 <- fav_states_scaled %>%
...
plot_trends_data <- function(states, path) {
  
  plot <- fav_states_scaled %>%
    plot_cases(scaled = T, title = "Confirmed Cases per 100,000 People vs Time")
  
  trends_data <- read.csv(path)
  
  plot <- plot + geom_line(data = trends_data,
                   inherit.aes = FALSE,
                   aes(x = as.Date(trends_data[, 1], "%Y-%m-%d"),
                       y = trends_data[, 2] * 25),
                   size = 2)
  return(plot)
}

path_1 <- "https://raw.githubusercontent.com/hybras/Covid-Analysis/legislation-overlay-scaled/data/covidTrendData.csv?token=AKOM6X7MITS3J2OPSUSE43272LGUO"
x <- plot_trends_data(fav_states, path_1)
x
