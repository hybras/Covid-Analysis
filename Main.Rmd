...
for (state in fav_states) {
  data_scaled <- merge(data_scaled,
                                    mutate(filter(data_fav_states, region == state),
                                           scaled_cases = (cases / filter(census, NAME == state)$POPESTIMATE2019) * 100000),
                                    all = TRUE)
}
```
