...
covid_map_state_1$Province_State <-
  tolower(covid_map_state_1$Province_State)
...
new_col_names_1 <-
  seq(as.Date("2020/1/21"), by = "day", length.out = 286) #get dates in date-time format
...
covid_cont_state.long_1 <- pivot_longer(
  covid_cont_states_1,
  cols = c(2:286),
  names_to = "date",
  values_to = "cases"
